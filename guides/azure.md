...
- The GPT Image options work as on OpenAI: `background: :transparent` (PNG output), `output_compression` with `output_format: :jpeg`, `input_fidelity` on edits, and `quality: :low | :medium | :high | :auto`. `moderation` is forwarded unchanged; Azure does not document it, so a rejection surfaces as an API error.
