defmodule ReqLLM.Providers.AnthropicTest do
  ...
  describe "decode_stream_event - text block boundaries" do
    alias ReqLLM.Providers.Anthropic.Response

    test "emits text_block_boundary marker for text content_block_start with index > 0" do
      {:ok, model} = ReqLLM.model("anthropic:claude-sonnet-4-5-20250929")

      # First text block (index 0) - no boundary marker
      event0 = %{
        data: %{
      ...
      chunks0 = Response.decode_stream_event(event0, model)
      assert [%ReqLLM.StreamChunk{type: :content, text: ""}] = chunks0

      # Second text block (index 1) - should emit boundary marker first
      event1 = %{
        data: %{
          "type" => "content_block_start",
          "index" => 1,
      ...
      chunks1 = Response.decode_stream_event(event1, model)
      assert [boundary, content] = chunks1
      assert %ReqLLM.StreamChunk{type: :meta, metadata: %{text_block_boundary: true}} = boundary
      assert %ReqLLM.StreamChunk{type: :content, text: ""} = content
    end

    test "web_search scenario: text before and after tool use gets separated" do
      {:ok, model} = ReqLLM.model("anthropic:claude-sonnet-4-5-20250929")
      context = %ReqLLM.Context{messages: []}

      # Simulate the stream events from a web_search response:
      ...
      chunks =
        events
        |> Enum.flat_map(&Response.decode_stream_event(&1, model))
