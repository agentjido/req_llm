defmodule LLMFixture do
  ...
  def step(provider, fixture_name) do
    ...
    fn request ->
      ...
      if live?() do
        ...
        # Tag the request and add a response step to capture the response
        request
  ...
  defp handle_replay(path) do
    ...
    {:ok,
     %Req.Response{
       status: resp["status"],
       headers: listify_headers(resp["headers"]),
  ...
  defp listify_headers(map), do: Enum.map(map, fn {k, v} -> {k, v} end)
