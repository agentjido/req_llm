defmodule ReqLLM.Provider.Utils do
  ...
  def prepare_options!(provider_mod, %ReqLLM.Model{} = model, user_opts) do
    supported_options = provider_mod.supported_provider_options()

    user_opts
    |> ReqLLM.Provider.Options.extract_generation_opts()
    |> reject_unknown_inline(supported_options)
    |> validate_subset_inline(supported_options)
    |> then(&Keyword.merge(provider_mod.default_provider_opts(), &1))
    |> Keyword.put(:model, model.model)
    |> maybe_put_context_inline(user_opts)
  end

  # Inlined helper functions for prepare_options!/3
  defp reject_unknown_inline(opts, allowed) do
  ...
  defp validate_subset_inline(opts, allowed_keys) do
    schema = ReqLLM.Provider.Options.generation_subset_schema(allowed_keys)
    NimbleOptions.validate!(opts, schema)
  end

  defp maybe_put_context_inline(opts, user_opts) do
