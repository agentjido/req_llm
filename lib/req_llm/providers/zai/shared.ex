defmodule ReqLLM.Providers.Zai.Shared do
  @moduledoc false
  ...
  def attach(provider_module, request, model_input, user_opts) do
  ...
  def decode_response({req, %{status: 200} = resp}, provider_id) do
    model =
      req.private[:req_llm_model] ||
        %LLMDB.Model{id: req.options[:model], provider: provider_id}
  ...
  def decode_response(request_response, _provider_id) do
  ...
  def encode_body(request) do
    body =
      case request.options[:operation] do
        :embedding -> encode_embedding_body(request)
        _ -> encode_zai_chat_body(request)
      end
  ...
  defp encode_zai_chat_body(request) do
    ...
    body =
      ...
      |> Utils.maybe_put(:stream, request.options[:stream])
      |> then(fn body ->
        if request.options[:stream],
          do: Map.put(body, :stream_options, %{include_usage: true}),
          else: body
      end)
      |> Utils.maybe_put(:max_tokens, request.options[:max_tokens])
  ...
  defp encode_embedding_body(request) do
    ...
    |> Utils.maybe_put(:user, request.options[:user])
    |> Utils.maybe_put(:dimensions, provider_opts[:dimensions])
    |> Utils.maybe_put(:encoding_format, provider_opts[:encoding_format])
  end
  ...
  defp add_basic_options(body, request_options) do
    ...
    Enum.reduce(body_options, body, fn key, acc ->
      Utils.maybe_put(acc, key, request_options[key])
    end)
  end
end
