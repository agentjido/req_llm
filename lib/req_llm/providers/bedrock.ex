defmodule ReqLLM.Providers.Bedrock do
  ...
  def attach_stream(model, context, opts, _finch_name) do
    ...
    error ->
      require Logger
      Logger.error("Error in attach_stream: #{Exception.message(error)}\nStacktrace: #{Exception.format_stacktrace(__STACKTRACE__)}")
  ...
  defp put_aws_sigv4(request, aws_creds) do
    ...
    |> Req.Request.prepend_request_steps(
      aws_sigv4: fn req ->
        ...
        headers = Map.new(req.headers, fn {k, v} ->
          {k, if(is_list(v), do: List.first(v), else: v)}
        end)
        body = req.body || ""

        signed_headers_list = AWSAuth.sign_authorization_header(
