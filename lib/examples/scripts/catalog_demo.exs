defmodule CatalogDemo do
  ...
  defp demonstrate_base_catalog do
    ...
    if base["openai"] do
      ...
      |> Enum.each(fn model_id ->
        ...
        if model["cost"] do
          IO.puts("      Cost: $#{model["cost"]["input"]}/M input, $#{model["cost"]["output"]}/M output")
        end
      end)
    end
  end
        ...
        if catalog["openai"] do
          ...
          if catalog["openai"]["models"]["gpt-4o-mini"] do
            ...
            if original_gpt4o_mini && original_gpt4o_mini["cost"] do
              IO.puts("    Original cost: $#{original_gpt4o_mini["cost"]["input"]}/$#{original_gpt4o_mini["cost"]["output"]} per M")
            end
            IO.puts("    Override cost: $#{model["cost"]["input"]}/$#{model["cost"]["output"]} per M")
          end
        end
  ...
  defp demonstrate_full_pipeline do
    ...
    IO.puts("  Allowlist: OpenAI (gpt-4o), Anthropic (claude-3-5-sonnet), Local (my-custom-model)")
